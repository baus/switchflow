...
($host, $path, $port, $method, $numheaders) = lib::HTTPTestLib::parseCommandline(@ARGV);
...
#
#
my $startline = "$method $path HTTP/1.0$CRLF";
my $testheader = "x-summitsage-test-header: this is a test header$CRLF";
...
for(my $i = $numheaders; $i > 0; $i--){
#  syswrite(STDOUT, $testheader, length($testheader));
  syswrite($sock, $testheader, length($testheader));
} 
syswrite(STDOUT, $CRLF, length($CRLF));
syswrite($sock, $CRLF, length($CRLF));
