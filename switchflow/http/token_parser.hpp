...
class token_parser
{
 public:
  ...
  parse_result parse(boost::asio::const_buffer buffer, token_receiver receiver)
  {
    parse_result result;
    ...
    for(i = 0; i < buffer_size && current_length_ < max_length_; ++i, ++current_length_){
      if(delimiters_.find(raw_buffer[i]) != delimiters_.end()){
        ...
        result.status = convert_to_parse_status(receiver(boost::asio::const_buffer(raw_buffer, i), raw_buffer[i], buf_status), COMPLETE);
        current_length_ = 0;
        return result;
      }
      //
    ...
    result.status = convert_to_parse_status(receiver(boost::asio::const_buffer(raw_buffer, i), raw_buffer[i], buf_status), INCOMPLETE);
